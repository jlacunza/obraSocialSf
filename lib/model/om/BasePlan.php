 ...
 */
abstract class BasePlan extends BaseObject 
{
    ...
    public function setDescripcion($v)
    {
        ...
        if ($this->descripcion !== $v) {
            $this->descripcion = $v;
            $this->modifiedColumns[] = PlanPeer::DESCRIPCION;
        }
    ...
    public function setDescuento($v)
    {
        ...
        if ($this->descuento !== $v) {
            $this->descuento = $v;
            $this->modifiedColumns[] = PlanPeer::DESCUENTO;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->descuento = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
            $this->resetModified();
            ...
            return $startcol + 3; // 3 = PlanPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(PlanPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(PlanPeer::DESCRIPCION)) {
            $modifiedColumns[':p' . $index++]  = '`DESCRIPCION`';
        }
        if ($this->isColumnModified(PlanPeer::DESCUENTO)) {
            $modifiedColumns[':p' . $index++]  = '`DESCUENTO`';
        }

        $sql = sprintf(
            'INSERT INTO `plan` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Plan'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Plan'][$this->getPrimaryKey()] = true;
        $keys = PlanPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getDescripcion(),
            $keys[2] => $this->getDescuento(),
        );
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) $this->setDescuento($arr[$keys[2]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PlanPeer::DESCRIPCION)) $criteria->add(PlanPeer::DESCRIPCION, $this->descripcion);
        if ($this->isColumnModified(PlanPeer::DESCUENTO)) $criteria->add(PlanPeer::DESCUENTO, $this->descuento);
     ...
     * If this Plan is new, it will return
     ...
     * Otherwise if this Plan is new, it will return
     * an empty collection; or if this Plan has previously
     * been saved, it will retrieve related Afiliados from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Plan.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      PropelPDO $con optional connection object
    ...
    {
        return (string) $this->exportTo(PlanPeer::DEFAULT_STRING_FORMAT);
    }
    ...
    public function __call($name, $params)
    {
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BasePlan:' . $name))
		{
