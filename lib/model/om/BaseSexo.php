...
/**
 * Base class that represents a row from the 'sexo' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseSexo extends BaseObject 
{
     ...
     * @return   Sexo The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[] = SexoPeer::ID;
        }
    ...
    public function setDescripcion($v)
    {
        ...
        if ($this->descripcion !== $v) {
            $this->descripcion = $v;
            $this->modifiedColumns[] = SexoPeer::DESCRIPCION;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 2; // 2 = SexoPeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating Sexo object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = SexoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = SexoQuery::create()
                ->filterByPrimaryKey($this->getPrimaryKey());
            $ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseSexo:delete:pre') as $callable)
			{
            ...
            if ($ret) {
                ...
                $this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseSexo:delete:post') as $callable)
				{
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseSexo:save:pre') as $callable)
			{
            ...
            if ($ret) {
                ...
                $this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseSexo:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

                SexoPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(SexoPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(SexoPeer::DESCRIPCION)) {
            $modifiedColumns[':p' . $index++]  = '`DESCRIPCION`';
        }

        $sql = sprintf(
            'INSERT INTO `sexo` (%s) VALUES (%s)',
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = SexoPeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = SexoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Sexo'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Sexo'][$this->getPrimaryKey()] = true;
        $keys = SexoPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SexoPeer::ID)) $criteria->add(SexoPeer::ID, $this->id);
        if ($this->isColumnModified(SexoPeer::DESCRIPCION)) $criteria->add(SexoPeer::DESCRIPCION, $this->descripcion);
    ...
    {
        $criteria = new Criteria(SexoPeer::DATABASE_NAME);
        $criteria->add(SexoPeer::ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of Sexo (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 Sexo Clone of current object.
     * @throws PropelException
     */
     ...
     * If this Sexo is new, it will return
     ...
     * Otherwise if this Sexo is new, it will return
     * an empty collection; or if this Sexo has previously
     * been saved, it will retrieve related Afiliados from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Sexo.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      PropelPDO $con optional connection object
    ...
    {
        return (string) $this->exportTo(SexoPeer::DEFAULT_STRING_FORMAT);
    }
    ...
    public function __call($name, $params)
    {
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseSexo:' . $name))
		{
