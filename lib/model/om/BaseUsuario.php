...
abstract class BaseUsuario extends BaseObject 
{
    ...
    public function setUser($v)
    {
        ...
        if ($this->user !== $v) {
            $this->user = $v;
            $this->modifiedColumns[] = UsuarioPeer::USER;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->rol_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
            $this->resetModified();
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aRol !== null && $this->rol_id !== $this->aRol->getId()) {
            $this->aRol = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UsuarioPeer::USER)) {
            $modifiedColumns[':p' . $index++]  = '`USER`';
        }
        if ($this->isColumnModified(UsuarioPeer::PASSWORD)) {
            $modifiedColumns[':p' . $index++]  = '`PASSWORD`';
        }
        if ($this->isColumnModified(UsuarioPeer::ROL_ID)) {
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) $this->setRolId($arr[$keys[3]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsuarioPeer::USER)) $criteria->add(UsuarioPeer::USER, $this->user);
